		...
		if(node->token.tkIdentifier == SLASH_tk){
			if(node->child1 != NULL)
                ...
                if(node->token.tkIdentifier == IDENT_tk){
                        ...
                        if(exists == -1){
                                printf("1 Variable '%s' On Line: %d Was Not Declared In This Scope.\n", node->token.tkString, node->token.lineNo);
                                exit(EXIT_FAILURE);
                        }
                }
                else{
                ...
                int exists = varExists(node->token.tkString);
                if(exists == -1){
                        printf("2 Variable '%s' On Line: %d Was Not Declared In This Scope.\n", node->token.tkString, node->token.lineNo);
                        exit(EXIT_FAILURE);
                }
        }
                ...
                int exists = varExists(node->token.tkString);
                if(exists == -1){
                        printf("3 Variable '%s' On Line: %d Was Not Declared In This Scope.\n", node->token.tkString, node->token.lineNo);
                        exit(EXIT_FAILURE);
                }
                if(node->child1 != NULL)
                semanticCheck(node->child1, count);
