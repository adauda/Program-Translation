...
static const char *tokenNames[] = {
		...
		[BEGIN_tk] = "BEGIN_tk", [END_tk] = "END_tk", [ITER_tk] = "ITER_tk", [VAR_tk] = "VAR_tk", [VOID_tk] = "VOID_tk", [INT_tk] = "INT_tk",
...
node_t *vars(int depth){
        ...
        if(tk.tkIdentifier == LET_tk){          /* Predict <vars> -> let Identifier = Integer <vars> */
				...
				if(tk.tkIdentifier == INT_tk || tk.tkIdentifier == NUM_tk){
					node->token = tk;
					tk = scan(inputfp, &lineNo);
					node->child1 = vars(depth);
...
node_t *r(int depth){
        ...
        else if(tk.tkIdentifier == INT_tk || tk.tkIdentifier == NUM_tk){             /* Predict <R> -> Integer */
                node->token = tk;
                tk = scan(inputfp, &lineNo);
                return node;
        }
